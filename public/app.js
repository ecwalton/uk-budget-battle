...
function groupCost(id, year, includePending = true) {
  return getGame(includePending).decisions.reduce((sum, ids, start) => {
    const c = CARDS.find(
      (c) => c.id === ids.find((x) => x.startsWith(id + ":")),
    );
    return sum + policyCost(c, year - start, state.sensitivity);
  }, 0);
}

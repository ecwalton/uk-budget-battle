...
function resultPage() {
  ...
  return `${header()}<main id="main" class="results"><div class="results-hero"><div><div class="eyebrow">YOUR TERM IS COMPLETE / ${SHOCKS[state.shock].name.toUpperCase()}</div><h1>${best}</h1><p class="lede">Five Budgets later, here’s what your decisions leave behind.</p><span class="legacy-verdict ${result.passed ? "met" : "missed"}">${result.passed ? "ALL GAME CHECKS MET" : "SOME GAME CHECKS MISSED"}</span></div><img class="legacy-prop" src="/assets/interiors/legacy.png" width="300" height="246" alt="A closed ledger marking your five-Budget record"></div><div class="result-summary"><div class="result-main"><span>UNDERLYING DEFICIT IMPROVEMENT IN YEAR 5</span><strong>${signed(result.annualImprovement)}<i>£bn</i></strong><p>Policy spending less tax receipts, before interest, against existing plans.</p></div><div class="score-checks">${[
    [
      result.fundingPass,
...
function render() {
  ...
  const screenKey = `${state.mode}:${state.stage}:${state.decisions.length}`;
  const enterScreen = screenKey !== lastScreenKey;
    ...
    state.mode === "intro"
      ...
      : state.mode === "results"
        ...
        : state.mode === "recap"
          ? recapPage()
          : game();
  if (enterScreen) document.querySelector("main")?.classList.add("screen-enter");
...
document.addEventListener("click", (event) => {
  ...
  if (el.dataset.control) {
    ...
    render();
    document.querySelector(`[data-envelope="${id}"]`)?.classList.add("just-changed");

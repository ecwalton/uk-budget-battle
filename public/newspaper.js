...
export function budgetStory(game, index) {
  ...
  const now = simulate(game).years[index].policyCosts,
    prior = simulate({ ...game, decisions: game.decisions.slice(0, index) })
      .years[index].policyCosts;
  const cost = ENVELOPES.reduce((s, c) => s + now[c.id] - prior[c.id], 0),
    revenue = -TAXES.reduce((s, c) => s + now[c.id] - prior[c.id], 0);
...
export function newspaperSVG(game) {
  ...
  const record = game.decisions
    ...
    .join("");
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="1720" viewBox="0 0 1200 1720"><rect width="1200" height="1720" fill="#f7f4eb"/><g fill="#26333d" font-family="Georgia, serif">${text("THE BUDGET BATTLE", 600, 105, 64, 'text-anchor="middle" font-weight="bold"')}${text("YOUR TERM IN PRINT", 600, 142, 17, 'text-anchor="middle" letter-spacing="5"')}${rule(167)}${text("FIVE BUDGETS · TEN YEARS OF CONSEQUENCES", 64, 199, 16)}${text(SHOCKS[game.shock].name.toUpperCase(), 1136, 199, 16, 'text-anchor="end"')}${rule(221)}${paragraph(title, 64, 294, 36, 53, 61)}${text(`Migration: ${result.migration.migration}; lifetime net cost change £${result.migration.lifetimeNetCostBn.toFixed(2)}bn (outside Budget score)`, 64, 395, 18)}${text(`Adult dependants per worker: ${result.migration.dependants}; lower-paid wage sensitivity: ${result.migration.lowerPaidWagePercent === null ? "not quantified" : result.migration.lowerPaidWagePercent.toFixed(3) + "% (illustrative)"}`, 64, 420, 17)}${text(`Term saving: ${amount(result.termImprovement)} / £100bn target · Year 5 target: £40bn`, 64, 445, 23)}${rule(473)}${text("YEAR 5 UNDERLYING IMPROVEMENT", 64, 512, 15)}${text(amount(result.annualImprovement), 64, 571, 49, 'fill="#852c37"')}${text("ACTUAL YEAR 5 BORROWING", 665, 512, 15)}${text(`£${fifth.borrowing.toFixed(1)}bn`, 665, 571, 49)}<rect x="588" y="594" width="522" height="189" fill="#eae5d9"/>${text("LEGACY YEARS", 620, 620, 14, 'fill="#777267"')}<path d="${path("baseline")}" fill="none" stroke="#979489" stroke-width="3" stroke-dasharray="8 7"/><path d="${path("borrowing")}" fill="none" stroke="#852c37" stroke-width="4"/>${years.map((y, i) => text(`Y${y.year}`, px(i), 801, 14, 'text-anchor="middle"')).join("")}${text("Annual borrowing: burgundy = your plan; dashed = existing plans", 64, 841, 17)}${record}${rule(1470)}${text(`Legacy underlying improvement: ${amount(result.legacyImprovement)} / £200bn target in years 6–10`, 64, 1508, 22)}${text(`Lowest capacity: ${result.worstService.toFixed(1)} pts · Lowest household score: ${result.worstPressure.toFixed(1)} pts`, 64, 1543, 21)}${text(`Model debt in year 10: ${legacy.debtRatio.toFixed(1)}% of GDP · Revenue outlook: ${game.sensitivity}`, 64, 1578, 19)}${rule(1602)}${text("ILLUSTRATIVE TRAINING GAME · NOT AN OFFICIAL FORECAST", 64, 1636, 16, 'fill="#852c37"')}${text("Improvement is before interest; borrowing includes interest. Floors: capacity −5; lowest two incomes −4; others −5.", 64, 1666, 16)}${text(`uk-budget-battle.openuk-co.workers.dev · ${SCENARIO.version}`, 64, 1693, 14)}</g></svg>`;
}
